...
const HeadDashboard = () => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(
      `wss://employee-management-system-chat-feature.onrender.com/ws/chat/?token=${token}`,
    );
  ...
  useEffect(() => {
    const fetchDepartmentData = async () => {
      try {
        ...
        // Calculate stats
        const pendingLeaveCount = deptLeaveList.filter(
          (l) => l.status === "pending",
        ).length;
        const approvedLeaves = deptLeaveList.filter(
          (l) => l.status === "approved",
        );
        ...
        const taskPending = formattedTasks.filter(
          (t) => t.status !== "completed",
        ).length;
        const taskCompleted = formattedTasks.filter(
          (t) => t.status === "completed",
        ).length;
  ...
  const StatCard = ({
    ...
    <div className="bg-white rounded-2xl shadow-md border border-slate-100 p-6 hover:shadow-xl transition-all duration-300">
      <div className="flex items-start justify-between">
        ...
        <div
          className={`w-16 h-16 rounded-2xl ${lightBg} flex items-center justify-center flex-shrink-0`}
        >
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-white">
      ...
      <main className="lg:ml-64 p-4 sm:p-6 lg:p-8 mt-12 md:mt-10 lg:mt-0">
        ...
        <div className="mb-4 sm:mb-6 md:mb-8">
          <div className="relative group rounded-2xl sm:rounded-3xl bg-gradient-to-br from-blue-600 via-blue-500 to-blue-400 px-4 sm:px-6 md:px-8 py-6 sm:py-7 md:py-8 text-white shadow-xl hover:shadow-2xl transition-all duration-500 border border-white/20 overflow-hidden">
            {/* Animated background elements */}
            <div className="absolute inset-0 overflow-hidden">
              <div className="absolute top-0 right-0 w-40 sm:w-56 md:w-72 h-40 sm:h-56 md:h-72 bg-white/10 rounded-full -mr-20 -mt-20 blur-3xl group-hover:scale-150 transition-transform duration-700"></div>
              <div className="absolute bottom-0 left-0 w-32 sm:w-40 md:w-48 h-32 sm:h-40 md:h-48 bg-white/5 rounded-full -ml-16 -mb-16 blur-3xl group-hover:scale-150 transition-transform duration-700 delay-100"></div>

              {/* Floating particles */}
              <div className="absolute top-1/4 left-1/4 w-1 h-1 bg-white/30 rounded-full animate-ping"></div>
              <div className="absolute bottom-1/3 right-1/4 w-1.5 h-1.5 bg-white/20 rounded-full animate-pulse delay-300"></div>
              <div className="absolute top-2/3 left-1/3 w-1 h-1 bg-white/25 rounded-full animate-ping delay-500"></div>
            </div>
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
              ...
              <div className="flex-1 w-full sm:w-auto">
                <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                  ...
                  <p className="text-[10px] sm:text-xs uppercase tracking-widest text-blue-100 font-semibold">
                ...
                <p className="text-blue-100 text-sm sm:text-base md:text-lg flex items-center gap-2 flex-wrap">
                  <span>Here's your department's performance overview</span>
              ...
              {/* ACTION BUTTONS: CHAT & REFRESH - Responsive */}
              <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                {/* --- CHAT BUTTON with enhanced animation --- */}
                <div className="relative group/chat flex-1 sm:flex-initial">
                  <button
                    onClick={() => navigate("/chat")}
                    className="w-full sm:w-auto flex items-center justify-center gap-1.5 sm:gap-2 bg-white/20 hover:bg-white/30 text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg transition-all duration-300 border border-white/10 backdrop-blur-md hover:scale-105 active:scale-95"
                  >
                    <BsChatDots className="w-4 h-4 sm:w-5 sm:h-5 group-hover/chat:rotate-12 transition-transform" />
                    <span className="font-bold text-xs sm:text-sm">Chat</span>
                  </button>

                  {unreadCount > 0 && (
                    <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center rounded-full border-2 border-blue-500 shadow-md animate-bounce">
                ...
                <button
                  ...
                  disabled={refreshing}
                  className="flex-1 sm:flex-initial px-3 sm:px-4 py-2 sm:py-2.5 bg-white/20 hover:bg-white/30 disabled:opacity-50 disabled:hover:bg-white/20 rounded-lg flex items-center justify-center gap-1.5 sm:gap-2 transition-all duration-300 hover:scale-105 active:scale-95"
                >
                  <FaSync
                    className={`w-3 h-3 sm:w-4 sm:h-4 ${refreshing ? "animate-spin" : "group-hover:rotate-180"} transition-all duration-500`}
                  />
            ...
            {/* Interactive progress bar (optional) */}
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-8">
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500 mt-1">
                Status breakdown of team leaves
              </p>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-8">
            <div className="mb-6">
              ...
              <p className="text-sm text-slate-500 mt-1">
                Overall task status breakdown from all employees
              </p>
            </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-3xl shadow-lg border border-slate-100 p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1">
                  {stats.totalEmployees} members in your department
                </p>
              </div>
              <button
                    ...
                    {stats.tasksPending + stats.tasksCompleted > 0
                      ? Math.round(
                          (stats.tasksCompleted /
                            (stats.tasksPending + stats.tasksCompleted)) *
                            100,
                        )
                      : 0}
                    %
