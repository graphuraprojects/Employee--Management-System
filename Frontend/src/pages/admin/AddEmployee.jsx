...
export default function AddEmployee() {
  ...
  const positionOptions =
    positionsByDepartment[formData.department] || defaultPositions;
    ...
    if (validateStep(4)) {
      ...
      try {
        ...
        if (err.message === "Network Error" || !err.response) {
          ...
          showToast(
            "Upload failed. Network connection weak. Please check your internet.",
            "error",
          );
        } else {
          showToast(
  ...
  const handleEmailEmployee = async () => {
    ...
    try {
      const response = await emailService.registraionEmail(
        formData,
        employeeId,
      );
      ...
      if (response && response.success) {
        showToast(
          `Email sent successfully to ${formData.personalEmail}`,
          "success",
        );
      }
    } catch (err) {
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-4 lg:gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-600 via-blue-500 to-blue-400 flex items-center justify-center shadow-lg">
                    <MdPersonAdd className="text-white text-2xl w-20" />
                  </div>
                  <div>
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      <label
                        className={`flex items-center justify-center gap-3 px-4 py-4 border-2 rounded-lg cursor-pointer transition-all ${
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">
                          Tax ({formData.taxApply || "0"}%):
                        </span>
                        <span className="text-sm font-semibold text-red-600">
                          -₹
                          {(
                            ((parseFloat(formData.baseSalary) || 0) *
                              (parseFloat(formData.taxApply) || 0)) /
                            100
                          ).toFixed(2)}
                        </span>
                      </div>
            ...
            {currentStep === 4 && !isSubmitted && (
              <div className="bg-white/95 backdrop-blur rounded-2xl shadow-lg border border-blue-100 p-4 sm:p-6 lg:p-8">
                ...
                <p className="text-sm text-gray-500 mb-4 sm:mb-6">
                  Bank details are optional but recommended for salary
                  processing
                </p>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-700">
                        Bank details are required for salary processing and
                        direct deposit. You can add or update these details
                        later from the employee profile.
                      </p>
                    </div>
                  </div>
                </div>
